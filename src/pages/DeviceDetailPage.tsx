...
export default function DeviceDetailPage() {
  ...
  useEffect(() => {
    ...
    const off = wsService.onMessage((msg) => {
      ...
      if (type === "event" && (event === "device:lastSeen" || event === "device:upsert") && evDid === did) { setDeviceDoc((prev: any) => { if (!prev) return prev; const incoming = (data || {}) as any; const prevAt = Number(prev?.lastSeen?.at || 0); /* device:upsert carries the full doc (its lastSeen may be newer); device:lastSeen carries {at, action, battery}. Newest wins — an admin-triggered upsert carries the DB's unchanged lastSeen and can never make the device look fresher. */ let lastSeen = prev.lastSeen; if (event === "device:upsert") { const inAt = Number(incoming?.lastSeen?.at || 0); if (inAt > prevAt) lastSeen = incoming.lastSeen; } else { const evAt = Number(incoming?.at || 0); if (evAt > prevAt) lastSeen = { ...(prev.lastSeen || {}), at: evAt, action: safeStr(incoming?.action || prev?.lastSeen?.action || ""), battery: typeof incoming?.battery === "number" ? incoming.battery : (prev?.lastSeen?.battery ?? -1) }; } const { at: _a, action: _b, battery: _c, ...rest } = event === "device:lastSeen" ? incoming : { ...incoming, at: undefined, action: undefined, battery: undefined }; return { ...prev, ...rest, lastSeen }; }); return; }
      ...
      if (event === "device:uninstalled" && evDid === did) { /* backend decided (7d silent + dead token + ≥2 rejections). Token is kept server-side now — do not fake-erase it here. */ setDeviceDoc((prev: any) => prev ? { ...prev, fcmStatus: "uninstalled", unreachableReason: null } : prev); if (alertActionRef.current === "check_online") { alertActionRef.current = "check_online_done"; showResult("App Uninstalled! (backend confirmed)"); logStatus("App Uninstalled!", "red"); } return; }
      if (event === "device:recovered" && evDid === did) { /* backend reversed an uninstall: device made contact or Firebase accepted a push */ const st = safeStr(data?.status || "offline"); const rs = data?.reason ?? null; setDeviceDoc((prev: any) => prev ? { ...prev, fcmStatus: st, unreachableReason: rs } : prev); logStatus("Device recovered — no longer uninstalled", "green"); return; }
      if (event === "check_online:result" && evDid === did) {
        if (alertActionRef.current === "check_online") {
          ...
          if (status === "online") { alertActionRef.current = "check_online_done"; const ts = Number(data?.checkedAt || Date.now()); setCheckedAt(ts); showResult("Device is Online!"); logStatus("Device Online", "green"); }
          else if (err === "missing_token") { alertActionRef.current = "check_online_done"; /* "uninstalled" ONLY if the backend says so — the old 24h-silence guess is gone */ const isRealUninstalled = safeStr(deviceRef.current?.fcmStatus || "") === "uninstalled"; if (isRealUninstalled) { showResult("App Uninstalled! (backend confirmed)"); logStatus("App Uninstalled!", "red"); } else { showResult("Token missing — backend has no token yet. App will sync it on its next contact; check again in a few minutes."); logStatus("Token missing", "red"); } }
          else if (/registration-token|invalid-registration/i.test(err)) { alertActionRef.current = "check_online_done"; /* Firebase rejected the token; backend keeps it and waits for the app to push a new one */ showResult("Token dead — Firebase rejected this token. If the app is alive it will push a new one automatically; check again in a few minutes."); logStatus("Token dead — resync pending", "red"); }
          else if (err) { alertActionRef.current = "check_online_done"; showResult(`Device Unreachable: ${err}`); logStatus("Device Unreachable", "red"); }
        }
        return;
      }
  ...
  async function handleCheckOnline() {
    ...
    /* 30s deadline — previously the alert waited forever when the device never answered */
    const myWindow = alertWindowRef.current;
    setTimeout(() => { if (alertActionRef.current === "check_online" && alertWindowRef.current === myWindow) { alertActionRef.current = "check_online_done"; setDevAlert({ message: "No response in 30 seconds — device is offline (phone off / no network / app not running).", startTime: myWindow }); logStatus("No response — offline", "red"); } }, 30000);
    ...
    catch (err: any) { const apiErr = safeStr(err?.response?.data?.error || ""); if (apiErr === "missing_token") { alertActionRef.current = "check_online_done"; /* "uninstalled" ONLY if the backend says so — no 24h guess */ const isRealUninstalled = safeStr(device?.fcmStatus || "") === "uninstalled"; if (isRealUninstalled) { showResult("App Uninstalled! (backend confirmed)"); logStatus("App Uninstalled!", "red"); } else { showResult("Token missing — backend has no token yet. App will sync it on its next contact; check again in a few minutes."); logStatus("Token missing", "red"); } } else if (/registration-token|invalid-registration/i.test(apiErr)) { alertActionRef.current = "check_online_done"; showResult("Token dead — Firebase rejected this token. If the app is alive it will push a new one automatically; check again in a few minutes."); logStatus("Token dead — resync pending", "red"); } else if (apiErr) { alertActionRef.current = "check_online_done"; showResult(`FCM Failed: ${apiErr}`); logStatus("FCM send failed", "red"); } }
  }
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                <table className="w-full border-collapse">
                  <tbody>
                    ...
                    <tr className="border-b border-gray-100"><td className="py-3 pl-4 align-top text-[13px] font-semibold text-gray-600">FCM Status</td><td className="py-3 pr-4 text-[13px]">{(() => { const s = safeStr(device?.fcmStatus || ""); const r = safeStr(device?.unreachableReason || ""); const e = safeStr(device?.fcmLastError || ""); if (s === "uninstalled") return <span className="font-semibold text-red-600">Uninstalled — App removed from device (backend confirmed)</span>; /* LINE 1 — is the device talking to us? From real contact (lastSeen) only, never from fcmStatus. */ const lsAt = Number(device?.lastSeen?.at || 0); const lsAgoMs = lsAt > 0 ? Date.now() - lsAt : -1; const live = lsAt <= 0 ? <span className="font-semibold text-gray-400">Never seen</span> : lsAgoMs < 10000 ? <span className="font-semibold text-green-600">● Online</span> : lsAgoMs < 20 * 60 * 1000 ? <span><span className="font-semibold text-gray-600">Last seen: </span><TimeAgo ts={lsAt} className="font-semibold text-red-500" /></span> : <span className="font-semibold text-red-500">Offline</span>; /* LINE 2 — can the backend push to it? Shown only when there is something to say. */ const push = (s === "offline" && r === "token_dead") ? <div className="mt-1 text-[12px] font-semibold text-orange-600">⚠ push: token dead — Firebase rejected it; resync pending{e === "invalid_token_format" ? " (malformed)" : ""}</div> : (s === "offline" && r === "token_missing") ? <div className="mt-1 text-[12px] font-semibold text-yellow-700">⚠ push: no token yet — app will sync on next contact</div> : (e === "messaging/sender-id-mismatch" || e === "messaging/third-party-auth-error" || e === "messaging/mismatched-credential") ? <div className="mt-1 text-[12px] font-semibold text-purple-600">⚠ push: config error — Firebase sender mismatch</div> : null; return <div>{live}{push}</div>; })()}</td></tr>
