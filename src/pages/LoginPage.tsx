  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#0a0a0a] px-4">
      <style>{`
        ...
        .glitch::before { content: 'CEH'; position: absolute; left: -3px; top: 0; color: #00eeff; clip-path: polygon(0 20%, 100% 20%, 100% 45%, 0 45%); opacity: 0.85; animation: glitch1 3s infinite; }
        .glitch::after { content: 'CEH'; position: absolute; left: 3px; top: 0; color: #ff003c; clip-path: polygon(0 55%, 100% 55%, 100% 75%, 0 75%); opacity: 0.8; animation: glitch2 3s infinite; }
        @keyframes slideUp { from { transform: translateX(-50%) translateY(20px); opacity: 0; } to { transform: translateX(-50%) translateY(0); opacity: 1; } }
        .login-input { width: 100%; height: 48px; background: #0d0d0d; border: 1px solid #2a2a2a; border-radius: 12px; padding: 0 16px; font-size: 14px; color: #fff; outline: none; transition: border-color 0.2s; font-family: inherit; }
      ...
      <div className="w-full max-w-[380px] rounded-2xl border border-[#1e1e1e] bg-[#111111] px-7 pb-7 pt-9 shadow-2xl">
        <div className="mb-1 text-center">
          <span className="glitch text-[64px] font-black tracking-widest text-white" style={{ fontFamily: "'Arial Black', Impact, sans-serif", lineHeight: 1 }}>CEH</span>
        </div>
        <div className="mb-1 text-center font-mono text-[11px] tracking-[5px] text-[#00eeff] opacity-80">WEB BACKEND</div>
        <div className="mb-6 text-center font-mono text-[11px] tracking-[2px] text-[#333]">// zero-trace.in //</div>
            ...
            {step === "token" && (
              <form onSubmit={handleProceed} className="space-y-4">
                <div>
                  ...
                  <input value={tokenInput} onChange={(e) => { setTokenInput(e.target.value); setError(null); }} placeholder="Enter your token..." autoFocus autoComplete="off" className="login-input" />
                </div>
                {error && <div className="rounded-xl border border-red-900/50 bg-[#1a0505] px-3 py-2.5 font-mono text-[12px] text-red-400">// ERROR: {error}</div>}
                <button type="submit" className="w-full rounded-xl bg-white py-3 text-[14px] font-black tracking-widest text-black transition hover:bg-gray-200 active:scale-[0.98]">PROCEED →</button>
              </form>
            )}
            ...
            {step === "pin" && (
              <form onSubmit={handleSignIn} className="space-y-4">
                ...
                <div>
                  ...
                  <input ref={pinRef} value={pin} onChange={(e) => { setPin(e.target.value); setError(null); }} type="password" placeholder="Enter PIN..." inputMode="numeric" className="login-input" />
                </div>
                {error && <div className="rounded-xl border border-red-900/50 bg-[#1a0505] px-3 py-2.5 font-mono text-[12px] text-red-400">// ERROR: {error}</div>}
                <div className="flex gap-3">
                  <button type="submit" disabled={saving} className="flex-1 rounded-xl bg-white py-3 text-[14px] font-black tracking-widest text-black transition hover:bg-gray-200 active:scale-[0.98] disabled:opacity-50">
                    {saving ? "SIGNING IN…" : "SIGN IN"}
                  </button>
                  <button type="button" onClick={() => { setStep("token"); setPin(""); setError(null); }} className="rounded-xl border border-[#2a2a2a] bg-transparent px-4 py-3 text-[13px] font-semibold text-[#666] hover:border-[#444] hover:text-[#999]">← Back</button>
                </div>
              </form>
            )}

            <div className="mt-5 space-y-2.5">
              <button type="button" onClick={() => setContactOpen(true)} className="w-full rounded-xl border border-green-500/40 bg-transparent py-3 text-[13px] font-bold text-green-400 transition hover:border-green-500 hover:bg-green-500/5 active:scale-[0.98]">Contact Us</button>
              <button type="button" onClick={openTelegram} className="w-full rounded-xl border border-blue-500/40 bg-transparent py-3 text-[13px] font-bold text-blue-400 transition hover:border-blue-500 hover:bg-blue-500/5 active:scale-[0.98]">Telegram Channel</button>
              <button type="button" onClick={openSupportBot} className={`w-full rounded-xl border py-3 text-[13px] font-bold transition active:scale-[0.98] ${supportBotUrl ? "border-sky-400/40 bg-transparent text-sky-400 hover:border-sky-400 hover:bg-sky-400/5" : "border-gray-700/40 bg-transparent text-gray-600 cursor-default"}`}>
                🤖 Support Bot{!supportBotUrl ? " — Soon" : ""}
              </button>
            </div>
            <div className="mt-5 text-center font-mono text-[11px] tracking-[2px] text-[#333]">VERSION {version}</div>
          </>
        )}
      </div>

      {contactOpen && (
        <div className="fixed inset-0 z-[999] flex items-end justify-center bg-black/60" onClick={() => setContactOpen(false)}>
          <div className="w-full max-w-[380px] rounded-t-2xl border-t border-[#222] bg-[#111] px-5 pb-8 pt-5" onClick={e => e.stopPropagation()}>
            <div className="mb-4 text-center font-mono text-[13px] font-bold tracking-widest text-white">// CONTACT US //</div>
            <div className="space-y-3">
              <button type="button" onClick={() => { setContactOpen(false); openWhatsApp(); }} className="w-full rounded-xl border border-green-500/40 py-3 text-[14px] font-bold text-green-400 hover:bg-green-500/5">WhatsApp</button>
              <button type="button" onClick={() => { setContactOpen(false); openTelegramTarget(); }} className="w-full rounded-xl border border-blue-500/40 py-3 text-[14px] font-bold text-blue-400 hover:bg-blue-500/5">Telegram</button>
              <button type="button" onClick={() => { setContactOpen(false); openSupportBot(); }} className={`w-full rounded-xl border py-3 text-[14px] font-bold ${supportBotUrl ? "border-sky-400/40 text-sky-400 hover:bg-sky-400/5" : "border-gray-700/40 text-gray-600 cursor-default"}`}>
                🤖 Support Bot{!supportBotUrl ? " — Soon" : ""}
              </button>
            </div>
          </div>
